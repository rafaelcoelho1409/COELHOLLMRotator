...
def resolve_key(env_name: str) -> str:
    ...
    if v and v.strip():
        return v.strip()
    alt = env_name.lower().replace("_", "-")
    v = os.getenv(alt)
    if v and v.strip():
        return v.strip()
    v = os.getenv(env_name.lower())
    if v and v.strip():
        return v.strip()
    return ""


def get_store():  # stub: BYOK/MinIO removed — selection lives in env/secret now
    class _NoStore:
        def read_settings(self, force: bool = False) -> dict:  # noqa: ARG002
            return {}
    return _NoStore()
...
def _get_router() -> Router:
    ...
    # LiteLLM langfuse integration disabled (OTel/LangFuse stripped for demo).
    return _router_instance
