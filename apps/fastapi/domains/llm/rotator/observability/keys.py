...
# LangFuse v3 OTLP ingester maps: gen_ai.prompt→input, gen_ai.completion→output,
# gen_ai.request.model→model, gen_ai.usage.*→usage, gen_ai.request.{temperature,top_p,max_tokens}→modelParameters.
...
# Custom (not yet in gen_ai semconv); LangFuse + Tempo surface under metadata.
...
# `gen_ai.*` names register as LangFuse generation observations; others = generic spans.
